/*!
	@file sumxfev.cpp
	@brief The test file file for SumXFeV.
	@details This is the implementation file for SumXFeV.
...
int testSumXFeV(int ArgC, char * ArgV[])
{
	Itn I = 3, J = 3;
	Rnd theRnd(1328395039);
//	Rnd theRnd;
//	Itn I = parseN(ArgV[1]);
//	Itn J = parseN(ArgV[2]);
	...
	{
		Prm p[3] = {tVars[6], tIfs[0], tIfs[1]};
		tIfs[2] = SumXiFeVi::def(Xv, tSys, p , 3);
	}

	tIfs[3] = IfteXbYiFeVi::def(Xv|Yv, tSys, tVars[9], tVars[7], tVars[8]);
	tIfs[4] = IfteXbYiFeVi::def(Xv|Yv, tSys, tVars[12], tVars[10], tVars[11]);
	{
		Prm p[3] = {tVars[13], tIfs[3], tIfs[4]};
		tIfs[5] = SumXiFeVi::def(Xv, tSys, p , 3);
	}

	tIfs[6] = BaddXiFeVi::def(Xv, tSys, tIfs[2], tIfs[5]);
...
//					v14.ValueRec().NextData(ss.SimulClk()) ?
	...
	RefHdl(v6, Vi, tSys, tVars[6].TermHdl);
	...
	RefHdl(v13, Vi, tSys, tVars[13].TermHdl);
...
//	XOUTPUT
	...
	while(ss.ExecClk() <= I)
	{
//		Sys::execDiffRndVars(tSys, theRnd);
		Sys::execSwapRndVars(tSys, theRnd);
//		XOUTPUT
		FUNCTION
...
//			Sys::simulDiffRndVar(tSys, theRnd);
			Sys::simulSwapRndVars(tSys, theRnd);
			#if SimulDnwd
