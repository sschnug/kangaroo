/*!
	@file lockoptcompatsszt.tcc
	@brief The source file for LockOptCompatSsZt template class.
	@details This is the source file for LockOptCompatSsZt template class.
...
#ifndef LockOptCompatSsZtTccIncluded
#define LockOptCompatSsZtTccIncluded
...
class LockOptCompatSsZt : public SsZt<zt>
{
	public:

		static Hdl def(Fss const theFss, Hdl const hdlSys, Prm const & prmTerm);				//!< Define a full swap.
		static LockOptCompatSsZt<zt,theOrd> const & refc(Hdl const hdlSys, Hdl const hdlSelc);	//!< Refer to a selc.
		static LockOptCompatSsZt<zt,theOrd> const * ptrc(Hdl const hdlSys, Hdl const hdlSelc);	//!< Point to a selc.
		static LockOptCompatSsZt<zt,theOrd> & refm(Hdl const hdlSys, Hdl const hdlSelc);		//!< Refer to a selc.
		static LockOptCompatSsZt<zt,theOrd> * ptrm(Hdl const hdlSys, Hdl const hdlSelc);		//!< Point to a selc.
		...
		static LockOptCompatSsZt<zt,theOrd> & cast(Selc & theSelc);				//!< Perform casting.
		static LockOptCompatSsZt<zt,theOrd> * cast(Selc * theSelc);				//!< Perform casting.
		static LockOptCompatSsZt<zt,theOrd> const & cast(Selc const & theSelc);	//!< Perform casting.
		static LockOptCompatSsZt<zt,theOrd> const * cast(Selc const * theSelc);	//!< Perform casting.
	...
	private:
		...
		LockOptCompatSsZt(Fss const theFss, Hdl const hdlSys,
								Prm * prmTerm, Dim const PrmCount);		//!< Constructor.
		LockOptCompatSsZt(LockOptCompatSsZt<zt,theOrd> const & that);	//!< Duplicator.
		LockOptCompatSsZt<zt,theOrd> const & operator =
							(LockOptCompatSsZt<zt,theOrd> const & that);//!< Assigner.
		~LockOptCompatSsZt();											//!< Destructor.
...
*/
template<typename zt, Ord theOrd>
inline Bln LockOptCompatSsZt<zt,theOrd>::suit(Selc const & theSelc)
{
	WatchError
	return theSelc.SelcCls == SelcStub< LockOptCompatSsZt<zt,theOrd> >::SelcCls;
	CatchError
}
...
*/
template<typename zt, Ord theOrd>
inline Bln LockOptCompatSsZt<zt,theOrd>::suit(Selc & theSelc)
{
	WatchError
	return theSelc.SelcCls == SelcStub< LockOptCompatSsZt<zt,theOrd> >::SelcCls;
	CatchError
...
*/
template<typename zt, Ord theOrd>
inline LockOptCompatSsZt<zt,theOrd> & LockOptCompatSsZt<zt,theOrd>::cast(Selc & theSelc)
{
	WatchError
	Wchk(theSelc.SelcCls != SelcStub< LockOptCompatSsZt<zt,theOrd> >::SelcCls) Wtxt(eWrongCls);

	return scast<LockOptCompatSsZt<zt,theOrd> &>(theSelc);
	CatchError
}
...
*/
template<typename zt, Ord theOrd>
inline LockOptCompatSsZt<zt,theOrd> const & LockOptCompatSsZt<zt,theOrd>::cast(Selc const & theSelc)
{
	WatchError
	Wchk(theSelc.SelcCls != SelcStub< LockOptCompatSsZt<zt,theOrd> >::SelcCls) Wtxt(eWrongCls);

	return scast<LockOptCompatSsZt<zt,theOrd> const &>(theSelc);
	CatchError
}
...
*/
template<typename zt, Ord theOrd>
inline LockOptCompatSsZt<zt,theOrd> const &
LockOptCompatSsZt<zt,theOrd>::refc(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
template<typename zt, Ord theOrd>
inline LockOptCompatSsZt<zt,theOrd> const *
LockOptCompatSsZt<zt,theOrd>::ptrc(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
inline LockOptCompatSsZt<zt,theOrd> &
LockOptCompatSsZt<zt,theOrd>::refm(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
template<typename zt, Ord theOrd>
inline LockOptCompatSsZt<zt,theOrd> *
LockOptCompatSsZt<zt,theOrd>::ptrm(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
inline LockOptCompatSsZt<zt,theOrd>::~LockOptCompatSsZt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template<typename zt, Ord theOrd>
LockOptCompatSsZt<zt,theOrd>::LockOptCompatSsZt(LockOptCompatSsZt<zt,theOrd> const & that) :
	SsZt<zt>(that)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
LockOptCompatSsZt<zt,theOrd> const &
LockOptCompatSsZt<zt,theOrd>::operator = (LockOptCompatSsZt<zt,theOrd> const & that)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
Hdl LockOptCompatSsZt<zt,theOrd>::def(Fss const theFss, Hdl const hdlSys,
	Prm const & prmTerm)
{
	WatchError
	Prm tPrms[1] = { prmTerm };
	LockOptCompatSsZt<zt,theOrd> * tSsX =
		new LockOptCompatSsZt<zt,theOrd>(theFss, hdlSys, tPrms, 1);
...
*/
template<typename zt, Ord theOrd>
LockOptCompatSsZt<zt,theOrd>::LockOptCompatSsZt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount) : SsZt<zt>(hdlSys, PrmCount)
{
	WatchError
	this->setSelcCls(SelcStub< LockOptCompatSsZt<zt,theOrd> >::SelcCls);
...
*/
template<typename zt, Ord theOrd>
void LockOptCompatSsZt<zt,theOrd>::require(Bll const SetNotUnset)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
Bll LockOptCompatSsZt<zt,theOrd>::select(Rnd & theRnd)
{
	WatchError
...
#endif	//LockOptCompatSsZtTccIncluded
