/*!
	@file dvtabuoptcompatsszt.tcc
	@brief The source file for DvTabuOptCompatSsZt template class.
	@details This is the source file for DvTabuOptCompatSsZt template class.
...
#ifndef DvTabuOptCompatSsZtTccIncluded
#define DvTabuOptCompatSsZtTccIncluded
...
class DvTabuOptCompatSsZt : public SsZt<zt>
{
	public:

		static Hdl def(Fss const theFss, Hdl const hdlSys, Prm const & prmTerm, Typ const typTabu);	//!< Define a full swap.
		static DvTabuOptCompatSsZt<zt,theOrd> const & refc(Hdl const hdlSys, Hdl const hdlSelc);//!< Refer to a selc.
		static DvTabuOptCompatSsZt<zt,theOrd> const * ptrc(Hdl const hdlSys, Hdl const hdlSelc);//!< Point to a selc.
		static DvTabuOptCompatSsZt<zt,theOrd> & refm(Hdl const hdlSys, Hdl const hdlSelc);		//!< Refer to a selc.
		static DvTabuOptCompatSsZt<zt,theOrd> * ptrm(Hdl const hdlSys, Hdl const hdlSelc);		//!< Point to a selc.
		...
		static DvTabuOptCompatSsZt<zt,theOrd> & cast(Selc & theSelc);				//!< Perform casting.
		static DvTabuOptCompatSsZt<zt,theOrd> * cast(Selc * theSelc);				//!< Perform casting.
		static DvTabuOptCompatSsZt<zt,theOrd> const & cast(Selc const & theSelc);	//!< Perform casting.
		static DvTabuOptCompatSsZt<zt,theOrd> const * cast(Selc const * theSelc);	//!< Perform casting.
	...
	private:
		...
		DvTabuOptCompatSsZt(Fss const theFss, Hdl const hdlSys,
					Prm * prmTerm, Dim const PrmCount, Typ const typTabu);		//!< Constructor.
		DvTabuOptCompatSsZt(DvTabuOptCompatSsZt<zt,theOrd> const & that);	//!< Duplicator.
		DvTabuOptCompatSsZt<zt,theOrd> const & operator =
							(DvTabuOptCompatSsZt<zt,theOrd> const & that);	//!< Assigner.
		~DvTabuOptCompatSsZt();												//!< Destructor.
...
*/
template<typename zt, Ord theOrd>
inline Bln DvTabuOptCompatSsZt<zt,theOrd>::suit(Selc const & theSelc)
{
	WatchError
	return theSelc.SelcCls == SelcStub< DvTabuOptCompatSsZt<zt,theOrd> >::SelcCls;
	CatchError
}
...
*/
template<typename zt, Ord theOrd>
inline Bln DvTabuOptCompatSsZt<zt,theOrd>::suit(Selc & theSelc)
{
	WatchError
	return theSelc.SelcCls == SelcStub< DvTabuOptCompatSsZt<zt,theOrd> >::SelcCls;
	CatchError
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptCompatSsZt<zt,theOrd> & DvTabuOptCompatSsZt<zt,theOrd>::cast(Selc & theSelc)
{
	WatchError
	Wchk(theSelc.SelcCls != SelcStub< DvTabuOptCompatSsZt<zt,theOrd> >::SelcCls) Wtxt(eWrongCls);

	return scast<DvTabuOptCompatSsZt<zt,theOrd> &>(theSelc);
	CatchError
}
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptCompatSsZt<zt,theOrd> const & DvTabuOptCompatSsZt<zt,theOrd>::cast(Selc const & theSelc)
{
	WatchError
	Wchk(theSelc.SelcCls != SelcStub< DvTabuOptCompatSsZt<zt,theOrd> >::SelcCls) Wtxt(eWrongCls);

	return scast<DvTabuOptCompatSsZt<zt,theOrd> const &>(theSelc);
	CatchError
}
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptCompatSsZt<zt,theOrd> const &
DvTabuOptCompatSsZt<zt,theOrd>::refc(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
template<typename zt, Ord theOrd>
inline DvTabuOptCompatSsZt<zt,theOrd> const *
DvTabuOptCompatSsZt<zt,theOrd>::ptrc(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptCompatSsZt<zt,theOrd> &
DvTabuOptCompatSsZt<zt,theOrd>::refm(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
template<typename zt, Ord theOrd>
inline DvTabuOptCompatSsZt<zt,theOrd> *
DvTabuOptCompatSsZt<zt,theOrd>::ptrm(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptCompatSsZt<zt,theOrd>::~DvTabuOptCompatSsZt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template<typename zt, Ord theOrd>
DvTabuOptCompatSsZt<zt,theOrd>::DvTabuOptCompatSsZt(DvTabuOptCompatSsZt<zt,theOrd> const & that) :
...
*/
template<typename zt, Ord theOrd>
DvTabuOptCompatSsZt<zt,theOrd> const &
DvTabuOptCompatSsZt<zt,theOrd>::operator = (DvTabuOptCompatSsZt<zt,theOrd> const & that)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
Hdl DvTabuOptCompatSsZt<zt,theOrd>::def(Fss const theFss, Hdl const hdlSys,
	Prm const & prmTerm, Typ const typTabu)
{
	WatchError
	Prm tPrms[1] = { prmTerm };
	DvTabuOptCompatSsZt<zt,theOrd> * tSsX =
		new DvTabuOptCompatSsZt<zt,theOrd>(theFss, hdlSys, tPrms, 1, typTabu);
...
*/
template<typename zt, Ord theOrd>
DvTabuOptCompatSsZt<zt,theOrd>::DvTabuOptCompatSsZt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount, Typ const typTabu) : SsZt<zt>(hdlSys, PrmCount),
		mSysTabu(DvTabu::refc(hdlSys,typTabu))
{
	WatchError
	this->setSelcCls(SelcStub< DvTabuOptCompatSsZt<zt,theOrd> >::SelcCls);
...
*/
template<typename zt, Ord theOrd>
void DvTabuOptCompatSsZt<zt,theOrd>::require(Bll const SetNotUnset)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
Bll DvTabuOptCompatSsZt<zt,theOrd>::select(Rnd & theRnd)
{
	WatchError
	...
	for(Sqn tSqn1 = 0; tSqn1 < tVarCount - 1; ++tSqn1)
	{
		...
		for(Sqn tSqn2 = tSqn1 + 1; tSqn2 < tVarCount; ++tSqn2)
		{
			...
			if (mSysTabu.state(tVarHdl1, tVarHdl2))
				continue;
...
#endif	//DvTabuOptCompatSsZtTccIncluded
