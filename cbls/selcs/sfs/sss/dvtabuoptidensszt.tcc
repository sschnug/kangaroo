/*!
	@file dvtabuoptidensszt.tcc
	@brief The source file for DvTabuOptIdenSsZt template class.
	@details This is the source file for DvTabuOptIdenSsZt template class.
...
#ifndef DvTabuOptIdenSsZtTccIncluded
#define DvTabuOptIdenSsZtTccIncluded
...
class DvTabuOptIdenSsZt : public SsZt<zt>
{
	public:

		static Hdl def(Fss const theFss, Hdl const hdlSys, Prm const & prmTerm, Typ const typTabu);	//!< Define a full swap.
		static DvTabuOptIdenSsZt<zt,theOrd> const & refc(Hdl const hdlSys, Hdl const hdlSelc);	//!< Refer to a selc.
		static DvTabuOptIdenSsZt<zt,theOrd> const * ptrc(Hdl const hdlSys, Hdl const hdlSelc);	//!< Point to a selc.
		static DvTabuOptIdenSsZt<zt,theOrd> & refm(Hdl const hdlSys, Hdl const hdlSelc);		//!< Refer to a selc.
		static DvTabuOptIdenSsZt<zt,theOrd> * ptrm(Hdl const hdlSys, Hdl const hdlSelc);		//!< Point to a selc.
		...
		static DvTabuOptIdenSsZt<zt,theOrd> & cast(Selc & theSelc);				//!< Perform casting.
		static DvTabuOptIdenSsZt<zt,theOrd> * cast(Selc * theSelc);				//!< Perform casting.
		static DvTabuOptIdenSsZt<zt,theOrd> const & cast(Selc const & theSelc);	//!< Perform casting.
		static DvTabuOptIdenSsZt<zt,theOrd> const * cast(Selc const * theSelc);	//!< Perform casting.
	...
	private:
		...
		DvTabuOptIdenSsZt(Fss const theFss, Hdl const hdlSys,
					Prm * prmTerm, Dim const PrmCount, Typ const typTabu);		//!< Constructor.
		DvTabuOptIdenSsZt(DvTabuOptIdenSsZt<zt,theOrd> const & that);	//!< Duplicator.
		DvTabuOptIdenSsZt<zt,theOrd> const & operator =
							(DvTabuOptIdenSsZt<zt,theOrd> const & that);	//!< Assigner.
		~DvTabuOptIdenSsZt();												//!< Destructor.
...
*/
template<typename zt, Ord theOrd>
inline Bln DvTabuOptIdenSsZt<zt,theOrd>::suit(Selc const & theSelc)
{
	WatchError
	return theSelc.SelcCls == SelcStub< DvTabuOptIdenSsZt<zt,theOrd> >::SelcCls;
	CatchError
}
...
*/
template<typename zt, Ord theOrd>
inline Bln DvTabuOptIdenSsZt<zt,theOrd>::suit(Selc & theSelc)
{
	WatchError
	return theSelc.SelcCls == SelcStub< DvTabuOptIdenSsZt<zt,theOrd> >::SelcCls;
	CatchError
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptIdenSsZt<zt,theOrd> & DvTabuOptIdenSsZt<zt,theOrd>::cast(Selc & theSelc)
{
	WatchError
	Wchk(theSelc.SelcCls != SelcStub< DvTabuOptIdenSsZt<zt,theOrd> >::SelcCls) Wtxt(eWrongCls);

	return scast<DvTabuOptIdenSsZt<zt,theOrd> &>(theSelc);
	CatchError
}
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptIdenSsZt<zt,theOrd> const & DvTabuOptIdenSsZt<zt,theOrd>::cast(Selc const & theSelc)
{
	WatchError
	Wchk(theSelc.SelcCls != SelcStub< DvTabuOptIdenSsZt<zt,theOrd> >::SelcCls) Wtxt(eWrongCls);

	return scast<DvTabuOptIdenSsZt<zt,theOrd> const &>(theSelc);
	CatchError
}
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptIdenSsZt<zt,theOrd> const &
DvTabuOptIdenSsZt<zt,theOrd>::refc(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
template<typename zt, Ord theOrd>
inline DvTabuOptIdenSsZt<zt,theOrd> const *
DvTabuOptIdenSsZt<zt,theOrd>::ptrc(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptIdenSsZt<zt,theOrd> &
DvTabuOptIdenSsZt<zt,theOrd>::refm(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
template<typename zt, Ord theOrd>
inline DvTabuOptIdenSsZt<zt,theOrd> *
DvTabuOptIdenSsZt<zt,theOrd>::ptrm(Hdl const hdlSys, Hdl const hdlSelc)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
inline DvTabuOptIdenSsZt<zt,theOrd>::~DvTabuOptIdenSsZt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template<typename zt, Ord theOrd>
DvTabuOptIdenSsZt<zt,theOrd>::DvTabuOptIdenSsZt(DvTabuOptIdenSsZt<zt,theOrd> const & that) :
...
*/
template<typename zt, Ord theOrd>
DvTabuOptIdenSsZt<zt,theOrd> const &
DvTabuOptIdenSsZt<zt,theOrd>::operator = (DvTabuOptIdenSsZt<zt,theOrd> const & that)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
Hdl DvTabuOptIdenSsZt<zt,theOrd>::def(Fss const theFss, Hdl const hdlSys,
	Prm const & prmTerm, Typ const typTabu)
{
	WatchError
	Prm tPrms[1] = { prmTerm };
	DvTabuOptIdenSsZt<zt,theOrd> * tSsX =
		new DvTabuOptIdenSsZt<zt,theOrd>(theFss, hdlSys, tPrms, 1, typTabu);
...
*/
template<typename zt, Ord theOrd>
DvTabuOptIdenSsZt<zt,theOrd>::DvTabuOptIdenSsZt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount, Typ const typTabu) : SsZt<zt>(hdlSys, PrmCount),
		mSysTabu(DvTabu::refc(hdlSys,typTabu))
{
	WatchError
	this->setSelcCls(SelcStub< DvTabuOptIdenSsZt<zt,theOrd> >::SelcCls);
...
*/
template<typename zt, Ord theOrd>
void DvTabuOptIdenSsZt<zt,theOrd>::require(Bll const SetNotUnset)
{
	WatchError
...
*/
template<typename zt, Ord theOrd>
Bll DvTabuOptIdenSsZt<zt,theOrd>::select(Rnd & theRnd)
{
	WatchError
...
#endif	//DvTabuOptIdenSsZtTccIncluded
