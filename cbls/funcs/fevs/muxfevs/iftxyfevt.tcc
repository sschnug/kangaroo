/*!
	@file iftxyfevt.tcc
	@brief The prototype file for IftXYFeVt template class.
	@details This is the prototype file for IftXYFeVt template class.
...
#ifndef IftXYFeVtTccIncluded
#define IftXYFeVtTccIncluded
...
class IftXYFeVt : public MuxFeVt<fvt>
{
	public:
		...
		static IftXYFeVt<pxt,pyt,fvt> & refm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Reference.
		static IftXYFeVt<pxt,pyt,fvt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Pointer.
		static IftXYFeVt<pxt,pyt,fvt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static IftXYFeVt<pxt,pyt,fvt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static IftXYFeVt<pxt,pyt,fvt> const & cast(Term const & theTerm);		//!< Perform casting.
		static IftXYFeVt<pxt,pyt,fvt> const * cast(Term const * theTerm);		//!< Perform casting.
		static IftXYFeVt<pxt,pyt,fvt> & cast(Term & theTerm);					//!< Perform casting.
		static IftXYFeVt<pxt,pyt,fvt> * cast(Term * theTerm);					//!< Perform casting.
	...
	protected:
		IftXYFeVt(Fss const theFss, Hdl const hdlSys,
						Prm * thePrms, Dim const PrmCount, pyt const & Else);	//!< Constructor.
		IftXYFeVt const & operator = (IftXYFeVt<pxt,pyt,fvt> const & that);		//!< Assigner.
		IftXYFeVt(IftXYFeVt<pxt,pyt,fvt> const & that);							//!< Duplicator.
		virtual ~IftXYFeVt();													//!< Destructor.
...
*/
template<typename pxt, typename pyt, typename fvt>
inline Bll IftXYFeVt<pxt,pyt,fvt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< IftXYFeVt<pxt,pyt,fvt> >::TermCls;
	CatchError
}
...
*/
template<typename pxt, typename pyt, typename fvt>
inline Bll IftXYFeVt<pxt,pyt,fvt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< IftXYFeVt<pxt,pyt,fvt> >::TermCls;
	CatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
inline IftXYFeVt<pxt,pyt,fvt> const & IftXYFeVt<pxt,pyt,fvt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< IftXYFeVt<pxt,pyt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<IftXYFeVt<pxt,pyt,fvt> const &>(theTerm);
	CatchError
}
...
*/
template<typename pxt, typename pyt, typename fvt>
inline IftXYFeVt<pxt,pyt,fvt> & IftXYFeVt<pxt,pyt,fvt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< IftXYFeVt<pxt,pyt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<IftXYFeVt<pxt,pyt,fvt> &>(theTerm);
	CatchError
}
...
*/
template<typename pxt, typename pyt, typename fvt>
inline IftXYFeVt<pxt,pyt,fvt> & IftXYFeVt<pxt,pyt,fvt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
inline IftXYFeVt<pxt,pyt,fvt> * IftXYFeVt<pxt,pyt,fvt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
inline IftXYFeVt<pxt,pyt,fvt> const & IftXYFeVt<pxt,pyt,fvt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
inline IftXYFeVt<pxt,pyt,fvt> const * IftXYFeVt<pxt,pyt,fvt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
inline IftXYFeVt<pxt,pyt,fvt>::~IftXYFeVt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
Hvl IftXYFeVt<pxt,pyt,fvt>::calcTermHvl() const
{
	WatchError
	Hvl tHvl = 0;
...
*/
template<typename pxt, typename pyt, typename fvt>
Bll IftXYFeVt<pxt,pyt,fvt>::equate(Term const & theTerm) const
{
	WatchError
	if (this->TermCls != theTerm.TermCls) return false;
	IftXYFeVt<pxt,pyt,fvt> const & tFeV = cast(theTerm);
...
*/
template<typename pxt, typename pyt, typename fvt>
inline void IftXYFeVt<pxt,pyt,fvt>::runPostExecAnew()
{
	WatchError
	#if ExecUpwd
...
*/
template<typename pxt, typename pyt, typename fvt>
inline void IftXYFeVt<pxt,pyt,fvt>::runPostExecIncr()
{
	WatchError
	#if ExecUpwd
	this->clearExecChgdPrms();
...
*/
template<typename pxt, typename pyt, typename fvt>
inline void IftXYFeVt<pxt,pyt,fvt>::runPostExecBklg()
{
	WatchError
	if (this->mValueRec->PrevDiff(this->mTermSys.ExecClk()))
		this->notifyEvalUndoArgs();
...
*/
template<typename pxt, typename pyt, typename fvt>
inline void IftXYFeVt<pxt,pyt,fvt>::runPostSimulAnew()
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
inline void IftXYFeVt<pxt,pyt,fvt>::runPostSimulIncr()
{
	WatchError
	this->clearSimulChgdPrms();
...
*/
template<typename pxt, typename pyt, typename fvt>
Hdl IftXYFeVt<pxt,pyt,fvt>::def(Fss const theFss, Hdl const hdlSys,
		Prm const & Cond, Prm const & Then, pyt const & Else)
{
	WatchError
	Prm tPrms[2] = { Cond, Then};
	IftXYFeVt<pxt,pyt,fvt> * tFeV = new IftXYFeVt<pxt,pyt,fvt>(theFss, hdlSys, tPrms, 2, Else);
...
*/
template<typename pxt, typename pyt, typename fvt>
IftXYFeVt<pxt,pyt,fvt>::IftXYFeVt(Fss const theFss, Hdl const hdlSys, Prm * thePrms,
		Dim const PrmCount, pyt const & Else) : MuxFeVt<fvt>(hdlSys, PrmCount), mEy(Else)
{
	WatchError
	this->setTermCls(TermStub< IftXYFeVt<pxt,pyt,fvt> >::TermCls);
...
*/
template<typename pxt, typename pyt, typename fvt>
IftXYFeVt<pxt,pyt,fvt>::IftXYFeVt(IftXYFeVt<pxt,pyt,fvt> const & that) :
...
*/
template<typename pxt, typename pyt, typename fvt>
IftXYFeVt<pxt,pyt,fvt> const & IftXYFeVt<pxt,pyt,fvt>::operator =
		(IftXYFeVt<pxt,pyt,fvt> const & that)
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::notifyDeferedPrms()
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::notifyEnforcedPrms()
{
	WatchError
	Term * const tPrmTerm = this->mPrmTerms[0];
	if (tPrmTerm->computable())	//	not Updatable to exclude vars
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::reinitEvalUndoPrms()
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::reiterEvalUndoPrms()
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::compConst()
{
	WatchError
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::execAnew()
{
	WatchError
	#if ExecDnwd
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::simulAnew()
{
	WatchError
	#if SimulDnwd
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::execBklg()
{
	WatchError
	Bll tNeedRecomp = false;
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template<typename pxt, typename pyt, typename fvt>
void IftXYFeVt<pxt,pyt,fvt>::emulBklg()
{
	WatchError
	Bll tNeedRecomp = false;
	if (this->mBacklogPrms.tagged(0))
