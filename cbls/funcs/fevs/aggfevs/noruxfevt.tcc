/*!
	@file noruxfevt.tcc
	@brief The prototype file for NoruXFeVt template class.
	@details This is the prototype file for NoruXFeVt template class.
...
#ifndef NoruXFeVtTccInlcluded
#define NoruXFeVtTccInlcluded
...
/*!
	@class NoruXFeVt
	@brief A class to represent nor (unsatisfaction) fevs.
	@details This class represents nor (unsatisfaction) fevs.
*/
template <typename pxt, typename fvt>
class NoruXFeVt : public AggFeVt<fvt>
{
	public:
		...
		static NoruXFeVt<pxt,fvt> & refm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Reference.
		static NoruXFeVt<pxt,fvt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Pointer.
		static NoruXFeVt<pxt,fvt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static NoruXFeVt<pxt,fvt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static NoruXFeVt<pxt,fvt> const & cast(Term const & theTerm);	//!< Perform casting.
		static NoruXFeVt<pxt,fvt> const * cast(Term const * theTerm);	//!< Perform casting.
		static NoruXFeVt<pxt,fvt> & cast(Term & theTerm);				//!< Perform casting.
		static NoruXFeVt<pxt,fvt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~NoruXFeVt();														//!< Destructor.
		NoruXFeVt(NoruXFeVt<pxt,fvt> const & that);							//!< Duplicator.
		NoruXFeVt(Fss const theFss, Hdl const hdlSys,
										Prm * thePrms, Dim const PrmCount);	//!< Constructor.
		NoruXFeVt const & operator = (NoruXFeVt<pxt,fvt> const & that);		//!< Assigner.
...
*/
template <typename pxt, typename fvt>
inline Bll NoruXFeVt<pxt,fvt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< NoruXFeVt<pxt,fvt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline Bll NoruXFeVt<pxt,fvt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< NoruXFeVt<pxt,fvt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fvt>
inline NoruXFeVt<pxt,fvt> const & NoruXFeVt<pxt,fvt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< NoruXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<NoruXFeVt<pxt,fvt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline NoruXFeVt<pxt,fvt> & NoruXFeVt<pxt,fvt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< NoruXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<NoruXFeVt<pxt,fvt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline NoruXFeVt<pxt,fvt> & NoruXFeVt<pxt,fvt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline NoruXFeVt<pxt,fvt> * NoruXFeVt<pxt,fvt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline NoruXFeVt<pxt,fvt> const & NoruXFeVt<pxt,fvt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline NoruXFeVt<pxt,fvt> const * NoruXFeVt<pxt,fvt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline NoruXFeVt<pxt,fvt>::~NoruXFeVt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fvt>
Hdl NoruXFeVt<pxt,fvt>::def(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	NoruXFeVt<pxt,fvt> * tFeV = new NoruXFeVt<pxt,fvt>(theFss,hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fvt>
NoruXFeVt<pxt,fvt>::NoruXFeVt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount) :
		AggFeVt<fvt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< NoruXFeVt<pxt,fvt> >::TermCls);
...
*/
template <typename pxt, typename fvt>
NoruXFeVt<pxt,fvt>::NoruXFeVt(NoruXFeVt const & that) : AggFeVt<fvt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
NoruXFeVt<pxt,fvt> const & NoruXFeVt<pxt,fvt>::operator = (NoruXFeVt<pxt,fvt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void NoruXFeVt<pxt,fvt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fvt>
void NoruXFeVt<pxt,fvt>::execAnew()
{
	WatchError
	cExecTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void NoruXFeVt<pxt,fvt>::simulAnew()
{
	WatchError
	Dim tSimulTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void NoruXFeVt<pxt,fvt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void NoruXFeVt<pxt,fvt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fvt>
void NoruXFeVt<pxt,fvt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fvt>
void NoruXFeVt<pxt,fvt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fvt>
void NoruXFeVt<pxt,fvt>::emulBklg()
{
	WatchError
	 cEmulTrue = cExecTrue;
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
		...
		else if (tc<pxt,Bll>::iof(tPxRec->LateDiff(this->mTermSys.EmulClk())))
