/*!
	@file anduxfevt.tcc
	@brief The prototype file for AnduXFeVt template class.
	@details This is the prototype file for AnduXFeVt template class.
...
#ifndef AnduXFeVtTccIncluded
#define AnduXFeVtTccIncluded
...
/*!
	@class AnduXFeVt
	@brief A class to represent and (unsatisfaction) fevs.
	@details This class represents and (unsatisfaction) fevs.
*/
template <typename pxt, typename fvt>
class AnduXFeVt : public AggFeVt<fvt>
{
	public:
		static Hdl def(Fss const theFss, Hdl const hdlSys,
												Prm * thePrms, Dim const PrmCount);		//!< Definition.

		static AnduXFeVt<pxt,fvt> & refm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Reference.
		static AnduXFeVt<pxt,fvt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Pointer.
		static AnduXFeVt<pxt,fvt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static AnduXFeVt<pxt,fvt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static AnduXFeVt<pxt,fvt> const & cast(Term const & theTerm);	//!< Perform casting.
		static AnduXFeVt<pxt,fvt> const * cast(Term const * theTerm);	//!< Perform casting.
		static AnduXFeVt<pxt,fvt> & cast(Term & theTerm);				//!< Perform casting.
		static AnduXFeVt<pxt,fvt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~AnduXFeVt();													//!< Destructor.
		AnduXFeVt(AnduXFeVt<pxt,fvt> const & that);						//!< Duplicator.
		AnduXFeVt(Fss const theFss, Hdl const hdlSys,
									Prm * thePrms, Dim const PrmCount);	//!< Constructor.
		AnduXFeVt const & operator = (AnduXFeVt<pxt,fvt> const & that);	//!< Assigner.
...
*/
template <typename pxt, typename fvt>
inline Bll AnduXFeVt<pxt,fvt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< AnduXFeVt<pxt,fvt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline Bll AnduXFeVt<pxt,fvt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< AnduXFeVt<pxt,fvt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fvt>
inline AnduXFeVt<pxt,fvt> const & AnduXFeVt<pxt,fvt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< AnduXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<AnduXFeVt<pxt,fvt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline AnduXFeVt<pxt,fvt> & AnduXFeVt<pxt,fvt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< AnduXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<AnduXFeVt<pxt,fvt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline AnduXFeVt<pxt,fvt> & AnduXFeVt<pxt,fvt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline AnduXFeVt<pxt,fvt> * AnduXFeVt<pxt,fvt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline AnduXFeVt<pxt,fvt> const & AnduXFeVt<pxt,fvt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline AnduXFeVt<pxt,fvt> const * AnduXFeVt<pxt,fvt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline AnduXFeVt<pxt,fvt>::~AnduXFeVt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fvt>
Hdl AnduXFeVt<pxt,fvt>::def(Fss const theFss, Hdl const hdlSys,
			Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	AnduXFeVt<pxt,fvt> * tFeV = new AnduXFeVt<pxt,fvt>(theFss, hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fvt>
AnduXFeVt<pxt,fvt>::AnduXFeVt(Fss const theFss, Hdl const hdlSys,
			Prm * thePrms, Dim const PrmCount) :
		AggFeVt<fvt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< AnduXFeVt<pxt,fvt> >::TermCls);
...
*/
template <typename pxt, typename fvt>
AnduXFeVt<pxt,fvt>::AnduXFeVt(AnduXFeVt const & that) : AggFeVt<fvt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
AnduXFeVt<pxt,fvt> const & AnduXFeVt<pxt,fvt>::operator = (AnduXFeVt<pxt,fvt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void AnduXFeVt<pxt,fvt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fvt>
void AnduXFeVt<pxt,fvt>::execAnew()
{
	WatchError
	cExecFalse = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void AnduXFeVt<pxt,fvt>::simulAnew()
{
	WatchError
	Dim tSimulFalse = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void AnduXFeVt<pxt,fvt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void AnduXFeVt<pxt,fvt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fvt>
void AnduXFeVt<pxt,fvt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fvt>
void AnduXFeVt<pxt,fvt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fvt>
void AnduXFeVt<pxt,fvt>::emulBklg()
{
	WatchError
