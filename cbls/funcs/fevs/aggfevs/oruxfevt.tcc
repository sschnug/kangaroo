/*!
	@file oruxfevt.tcc
	@brief The prototype file for OruXFeVt template class.
	@details This is the prototype file for OruXFeVt template class.
...
#ifndef OruXFeVtTccInlcluded
#define OruXFeVtTccInlcluded
...
/*!
	@class OruXFeVt
	@brief A class to represent or (unsatisfaction) fevs.
	@details This class represents or (unsatisfaction) fevs.
*/
template <typename pxt, typename fvt>
class OruXFeVt : public AggFeVt<fvt>
{
	public:

		static Hdl def(Fss const theFss, Hdl const hdlSys,
												Prm * thePrms, Dim const PrmCount);	//!< Definition.
		static OruXFeVt<pxt,fvt> & refm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Reference.
		static OruXFeVt<pxt,fvt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Pointer.
		static OruXFeVt<pxt,fvt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static OruXFeVt<pxt,fvt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static OruXFeVt<pxt,fvt> const & cast(Term const & theTerm);	//!< Perform casting.
		static OruXFeVt<pxt,fvt> const * cast(Term const * theTerm);	//!< Perform casting.
		static OruXFeVt<pxt,fvt> & cast(Term & theTerm);				//!< Perform casting.
		static OruXFeVt<pxt,fvt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~OruXFeVt();													//!< Destructor.
		OruXFeVt(OruXFeVt<pxt,fvt> const & that);						//!< Duplicator.
		OruXFeVt(Fss const theFss, Hdl const hdlSys,
								Prm * thePrms, Dim const PrmCount);		//!< Constructor.
		OruXFeVt const & operator = (OruXFeVt<pxt,fvt> const & that);	//!< Assigner.
...
*/
template <typename pxt, typename fvt>
inline Bll OruXFeVt<pxt,fvt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< OruXFeVt<pxt,fvt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline Bll OruXFeVt<pxt,fvt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< OruXFeVt<pxt,fvt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fvt>
inline OruXFeVt<pxt,fvt> const & OruXFeVt<pxt,fvt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< OruXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<OruXFeVt<pxt,fvt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline OruXFeVt<pxt,fvt> & OruXFeVt<pxt,fvt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< OruXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<OruXFeVt<pxt,fvt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline OruXFeVt<pxt,fvt> & OruXFeVt<pxt,fvt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline OruXFeVt<pxt,fvt> * OruXFeVt<pxt,fvt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline OruXFeVt<pxt,fvt> const & OruXFeVt<pxt,fvt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline OruXFeVt<pxt,fvt> const * OruXFeVt<pxt,fvt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline OruXFeVt<pxt,fvt>::~OruXFeVt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fvt>
Hdl OruXFeVt<pxt,fvt>::def(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	OruXFeVt<pxt,fvt> * tFeV = new OruXFeVt<pxt,fvt>(theFss,hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fvt>
OruXFeVt<pxt,fvt>::OruXFeVt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount) :
		AggFeVt<fvt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< OruXFeVt<pxt,fvt> >::TermCls);
...
*/
template <typename pxt, typename fvt>
OruXFeVt<pxt,fvt>::OruXFeVt(OruXFeVt const & that) : AggFeVt<fvt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
OruXFeVt<pxt,fvt> const & OruXFeVt<pxt,fvt>::operator = (OruXFeVt<pxt,fvt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void OruXFeVt<pxt,fvt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fvt>
void OruXFeVt<pxt,fvt>::execAnew()
{
	WatchError
	cExecTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void OruXFeVt<pxt,fvt>::simulAnew()
{
	WatchError
	Dim tSimulTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void OruXFeVt<pxt,fvt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void OruXFeVt<pxt,fvt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fvt>
void OruXFeVt<pxt,fvt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fvt>
void OruXFeVt<pxt,fvt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fvt>
void OruXFeVt<pxt,fvt>::emulBklg()
{
	WatchError
	 cEmulTrue = cExecTrue;
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
