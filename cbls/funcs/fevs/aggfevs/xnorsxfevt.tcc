/*!
	@file xnorsxfevt.tcc
	@brief The prototype file for XnorsXFeVt template class.
	@details This is the prototype file for XnorsXFeVt template class.
...
#ifndef XnorsXFeVtTccInlcluded
#define XnorsXFeVtTccInlcluded
...
class XnorsXFeVt : public AggFeVt<fvt>
{
	public:

		static Hdl def(Fss const theFss, Hdl const hdlSys,
												Prm * thePrms, Dim const PrmCount);		//!< Definition.
		static XnorsXFeVt<pxt,fvt> & refm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Reference.
		static XnorsXFeVt<pxt,fvt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Pointer.
		static XnorsXFeVt<pxt,fvt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static XnorsXFeVt<pxt,fvt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static XnorsXFeVt<pxt,fvt> const & cast(Term const & theTerm);	//!< Perform casting.
		static XnorsXFeVt<pxt,fvt> const * cast(Term const * theTerm);	//!< Perform casting.
		static XnorsXFeVt<pxt,fvt> & cast(Term & theTerm);				//!< Perform casting.
		static XnorsXFeVt<pxt,fvt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~XnorsXFeVt();														//!< Destructor.
		XnorsXFeVt(XnorsXFeVt<pxt,fvt> const & that);						//!< Duplicator.
		XnorsXFeVt(Fss const theFss, Hdl const hdlSys,
										Prm * thePrms, Dim const PrmCount);	//!< Constructor.
		XnorsXFeVt const & operator = (XnorsXFeVt<pxt,fvt> const & that);	//!< Assigner.
...
*/
template <typename pxt, typename fvt>
inline Bll XnorsXFeVt<pxt,fvt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< XnorsXFeVt<pxt,fvt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline Bll XnorsXFeVt<pxt,fvt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< XnorsXFeVt<pxt,fvt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fvt>
inline XnorsXFeVt<pxt,fvt> const & XnorsXFeVt<pxt,fvt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< XnorsXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<XnorsXFeVt<pxt,fvt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline XnorsXFeVt<pxt,fvt> & XnorsXFeVt<pxt,fvt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< XnorsXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<XnorsXFeVt<pxt,fvt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline XnorsXFeVt<pxt,fvt> & XnorsXFeVt<pxt,fvt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline XnorsXFeVt<pxt,fvt> * XnorsXFeVt<pxt,fvt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline XnorsXFeVt<pxt,fvt> const & XnorsXFeVt<pxt,fvt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline XnorsXFeVt<pxt,fvt> const * XnorsXFeVt<pxt,fvt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline XnorsXFeVt<pxt,fvt>::~XnorsXFeVt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fvt>
Hdl XnorsXFeVt<pxt,fvt>::def(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	XnorsXFeVt<pxt,fvt> * tFeV = new XnorsXFeVt<pxt,fvt>(theFss,hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fvt>
XnorsXFeVt<pxt,fvt>::XnorsXFeVt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount) :
		AggFeVt<fvt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< XnorsXFeVt<pxt,fvt> >::TermCls);
...
*/
template <typename pxt, typename fvt>
XnorsXFeVt<pxt,fvt>::XnorsXFeVt(XnorsXFeVt const & that) : AggFeVt<fvt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
XnorsXFeVt<pxt,fvt> const & XnorsXFeVt<pxt,fvt>::operator = (XnorsXFeVt<pxt,fvt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void XnorsXFeVt<pxt,fvt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fvt>
void XnorsXFeVt<pxt,fvt>::execAnew()
{
	WatchError
	cExecTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void XnorsXFeVt<pxt,fvt>::simulAnew()
{
	WatchError
	Dim tSimulTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void XnorsXFeVt<pxt,fvt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void XnorsXFeVt<pxt,fvt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fvt>
void XnorsXFeVt<pxt,fvt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fvt>
void XnorsXFeVt<pxt,fvt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fvt>
void XnorsXFeVt<pxt,fvt>::emulBklg()
{
	WatchError
	 cEmulTrue = cExecTrue;
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
