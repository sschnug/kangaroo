/*!
	@file fcountxfevt.tcc
	@brief The prototype file for FcountXFeVt template class.
	@details This is the prototype file for FcountXFeVt template class.
...
#ifndef FcountXFeVtTccIncluded
#define FcountXFeVtTccIncluded
...
/*!
	@class FcountXFeVt
	@brief A class to represent false count fevs.
	@details This class represents false count fevs.
*/
template <typename pxt, typename fvt>
class FcountXFeVt : public AggFeVt<fvt>
{
	public:
		static Hdl def(Fss const theFss, Hdl const hdlSys,
												Prm * thePrms, Dim const PrmCount);	//!< Definition.

		static FcountXFeVt<pxt,fvt> & refm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Reference.
		static FcountXFeVt<pxt,fvt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Pointer.
		static FcountXFeVt<pxt,fvt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static FcountXFeVt<pxt,fvt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static FcountXFeVt<pxt,fvt> const & cast(Term const & theTerm);	//!< Perform casting.
		static FcountXFeVt<pxt,fvt> const * cast(Term const * theTerm);	//!< Perform casting.
		static FcountXFeVt<pxt,fvt> & cast(Term & theTerm);				//!< Perform casting.
		static FcountXFeVt<pxt,fvt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~FcountXFeVt();													//!< Destructor.
		FcountXFeVt(FcountXFeVt<pxt,fvt> const & that);						//!< Duplicator.
		FcountXFeVt(Fss const theFss, Hdl const hdlSys,
									Prm * thePrms, Dim const PrmCount);	//!< Constructor.
		FcountXFeVt const & operator = (FcountXFeVt<pxt,fvt> const & that);	//!< Assigner.
...
*/
template <typename pxt, typename fvt>
inline Bll FcountXFeVt<pxt,fvt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< FcountXFeVt<pxt,fvt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline Bll FcountXFeVt<pxt,fvt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< FcountXFeVt<pxt,fvt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fvt>
inline FcountXFeVt<pxt,fvt> const & FcountXFeVt<pxt,fvt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< FcountXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<FcountXFeVt<pxt,fvt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline FcountXFeVt<pxt,fvt> & FcountXFeVt<pxt,fvt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< FcountXFeVt<pxt,fvt> >::TermCls) Wtxt(eWrongCls);

	return scast<FcountXFeVt<pxt,fvt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fvt>
inline FcountXFeVt<pxt,fvt> & FcountXFeVt<pxt,fvt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline FcountXFeVt<pxt,fvt> * FcountXFeVt<pxt,fvt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline FcountXFeVt<pxt,fvt> const & FcountXFeVt<pxt,fvt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline FcountXFeVt<pxt,fvt> const * FcountXFeVt<pxt,fvt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
inline FcountXFeVt<pxt,fvt>::~FcountXFeVt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fvt>
Hdl FcountXFeVt<pxt,fvt>::def(Fss const theFss, Hdl const hdlSys,
			Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	FcountXFeVt<pxt,fvt> * tFeV = new FcountXFeVt<pxt,fvt>(theFss, hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fvt>
FcountXFeVt<pxt,fvt>::FcountXFeVt(Fss const theFss, Hdl const hdlSys,
			Prm * thePrms, Dim const PrmCount) :
		AggFeVt<fvt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< FcountXFeVt<pxt,fvt> >::TermCls);
...
*/
template <typename pxt, typename fvt>
FcountXFeVt<pxt,fvt>::FcountXFeVt(FcountXFeVt const & that) : AggFeVt<fvt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
FcountXFeVt<pxt,fvt> const & FcountXFeVt<pxt,fvt>::operator = (FcountXFeVt<pxt,fvt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void FcountXFeVt<pxt,fvt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fvt>
void FcountXFeVt<pxt,fvt>::execAnew()
{
	WatchError
	cExecFalse = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void FcountXFeVt<pxt,fvt>::simulAnew()
{
	WatchError
	Dim tSimulFalse = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fvt>
void FcountXFeVt<pxt,fvt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fvt>
void FcountXFeVt<pxt,fvt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fvt>
void FcountXFeVt<pxt,fvt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fvt>
void FcountXFeVt<pxt,fvt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fvt>
void FcountXFeVt<pxt,fvt>::emulBklg()
{
	WatchError
