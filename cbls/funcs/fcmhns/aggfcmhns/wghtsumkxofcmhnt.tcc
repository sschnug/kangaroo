/*!
	@file wghtsumkxofcmhnt.tcc
	@brief The prototype file for WghtSumKXOFcMHnT template class.
	@details This is the prototype file for WghtSumKXOFcMHnT template class.
...
#ifndef WghtSumKXOFcMHnTTccIncluded
#define WghtSumKXOFcMHnTTccIncluded
...
class WghtSumKXOFcMHnT : public AggOFcMHnT<fmt>
{
	public:
		...
		static WghtSumKXOFcMHnT<pkt,pxt,fmt> & refm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Reference.
		static WghtSumKXOFcMHnT<pkt,pxt,fmt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Pointer.
		static WghtSumKXOFcMHnT<pkt,pxt,fmt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static WghtSumKXOFcMHnT<pkt,pxt,fmt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static WghtSumKXOFcMHnT<pkt,pxt,fmt> const & cast(Term const & theTerm);	//!< Perform casting.
		static WghtSumKXOFcMHnT<pkt,pxt,fmt> const * cast(Term const * theTerm);	//!< Perform casting.
		static WghtSumKXOFcMHnT<pkt,pxt,fmt> & cast(Term & theTerm);				//!< Perform casting.
		static WghtSumKXOFcMHnT<pkt,pxt,fmt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		WghtSumKXOFcMHnT(Fss const theFss, Hdl const hdlSys,
						pkt * theWghts,	Prm * thePrms, Dim const PrmCount);					//!< Constructor.
		WghtSumKXOFcMHnT(WghtSumKXOFcMHnT<pkt,pxt,fmt> const & that);						//!< Duplicator.
		WghtSumKXOFcMHnT const & operator = (WghtSumKXOFcMHnT<pkt,pxt,fmt> const & that);	//!< Assigner.
		~WghtSumKXOFcMHnT();																//!< Destructor.
...
*/
template <typename pkt, typename pxt, typename fmt>
inline Bll WghtSumKXOFcMHnT<pkt,pxt,fmt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< WghtSumKXOFcMHnT<pkt,pxt,fmt> >::TermCls;
	CatchError
}
...
*/
template <typename pkt, typename pxt, typename fmt>
inline Bll WghtSumKXOFcMHnT<pkt,pxt,fmt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< WghtSumKXOFcMHnT<pkt,pxt,fmt> >::TermCls;
	CatchError
...
*/
template <typename pkt, typename pxt, typename fmt>
inline WghtSumKXOFcMHnT<pkt,pxt,fmt> const & WghtSumKXOFcMHnT<pkt,pxt,fmt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< WghtSumKXOFcMHnT<pkt,pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<WghtSumKXOFcMHnT<pkt,pxt,fmt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pkt, typename pxt, typename fmt>
inline WghtSumKXOFcMHnT<pkt,pxt,fmt> & WghtSumKXOFcMHnT<pkt,pxt,fmt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< WghtSumKXOFcMHnT<pkt,pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<WghtSumKXOFcMHnT<pkt,pxt,fmt> &>(theTerm);
	CatchError
}
...
*/
template <typename pkt, typename pxt, typename fmt>
inline WghtSumKXOFcMHnT<pkt,pxt,fmt> & WghtSumKXOFcMHnT<pkt,pxt,fmt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pkt, typename pxt, typename fmt>
inline WghtSumKXOFcMHnT<pkt,pxt,fmt> * WghtSumKXOFcMHnT<pkt,pxt,fmt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pkt, typename pxt, typename fmt>
inline WghtSumKXOFcMHnT<pkt,pxt,fmt> const & WghtSumKXOFcMHnT<pkt,pxt,fmt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pkt, typename pxt, typename fmt>
inline WghtSumKXOFcMHnT<pkt,pxt,fmt> const * WghtSumKXOFcMHnT<pkt,pxt,fmt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pkt, typename pxt, typename fmt>
inline WghtSumKXOFcMHnT<pkt,pxt,fmt>::~WghtSumKXOFcMHnT()
{
	WatchError
...
*/
template <typename pkt, typename pxt, typename fmt>
Hdl WghtSumKXOFcMHnT<pkt,pxt,fmt>::def(Fss const theFss, Hdl const hdlSys, pkt * theWghts, Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 1 || PrmCount > MaxDim, eParamCount2(1, MaxDim));

	WghtSumKXOFcMHnT<pkt,pxt,fmt> * tFcMHn = new WghtSumKXOFcMHnT<pkt,pxt,fmt>(theFss, hdlSys, theWghts, thePrms, PrmCount);
...
*/
template <typename pkt, typename pxt, typename fmt>
WghtSumKXOFcMHnT<pkt,pxt,fmt>::WghtSumKXOFcMHnT(Fss const theFss, Hdl const hdlSys,
		pkt * theWghts, Prm * thePrms, Dim const PrmCount) :
		AggOFcMHnT<fmt>(hdlSys, PrmCount), mPxRecs(PrmCount),
		mPoRecs(PrmCount), mPrOpfs(PrmCount), mPkWghts(PrmCount),
		cExecHintMin(Null), sChgdHintMin(Null)
{
	WatchError
	this->setTermCls(TermStub< WghtSumKXOFcMHnT<pkt,pxt,fmt> >::TermCls);
...
*/
template <typename pkt, typename pxt, typename fmt>
WghtSumKXOFcMHnT<pkt,pxt,fmt>::WghtSumKXOFcMHnT(WghtSumKXOFcMHnT const & that) : AggOFcMHnT<fmt>(that),
		mPxRecs(0), mPoRecs(0), mPrOpfs(0), mPkWghts(0)
{
	WatchError
...
*/
template <typename pkt, typename pxt, typename fmt>
WghtSumKXOFcMHnT<pkt,pxt,fmt> const & WghtSumKXOFcMHnT<pkt,pxt,fmt>::operator =
		(WghtSumKXOFcMHnT<pkt,pxt,fmt> const & that)
{
	WatchError
...
*/
template <typename pkt, typename pxt, typename fmt>
Hvl WghtSumKXOFcMHnT<pkt,pxt,fmt>::calcTermHvl() const
{
	WatchError
	Hvl tHvl = 0;
...
*/
template <typename pkt, typename pxt, typename fmt>
Bln WghtSumKXOFcMHnT<pkt,pxt,fmt>::equate(Term const & theTerm) const
{
	WatchError
	if (this->TermCls != theTerm.TermCls) return false;
	WghtSumKXOFcMHnT<pkt,pxt,fmt> const & tFunc = cast(theTerm);
...
*/
template <typename pkt, typename pxt, typename fmt>
void WghtSumKXOFcMHnT<pkt,pxt,fmt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pkt, typename pxt, typename fmt>
void WghtSumKXOFcMHnT<pkt,pxt,fmt>::execAnew()
{
	WatchError
	cExecMetric = 0;
	for(Sqn tSqn = 0; tSqn < this->FuncVars.itemCount(); ++tSqn)
...
*/
template <typename pkt, typename pxt, typename fmt>
void WghtSumKXOFcMHnT<pkt,pxt,fmt>::simulAnew()
{
	WatchError
	...
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
	{
		...
		tSimulMetric += mPkWghts[tPos] *
				mPxRecs[tPos]->NextData(this->mTermSys.SimulClk());
	}
...
*/
template <typename pkt, typename pxt, typename fmt>
void WghtSumKXOFcMHnT<pkt,pxt,fmt>::undoEval(Pos const thePos)
{
	WatchError
	...
	if (!this->mHintNotEvals[thePos])
	{
		...
		for(Sqn tPrmSqn = 0; tPrmSqn < this->PrmVars[thePos].itemCount(); ++tPrmSqn)
		{
			...
			cExecHintMin->item(tSqn) -= mPkWghts[thePos] *
						mPrOpfs[thePos](tPeRec->PrevData(this->mTermSys.ExecClk()));
		}
	}
	CatchError
}
...
*/
template <typename pkt, typename pxt, typename fmt>
void WghtSumKXOFcMHnT<pkt,pxt,fmt>::undoHint(Pos const thePos)
{
	WatchError
	if (this->mHintNotEvals[thePos])
	{
		...
		for (Idx tIdx = 0; tIdx < tPhRec->DiffSqns.itemCount(); ++tIdx)
		{
			...
			if (this->mHintUndonePrms->list(thePos).tagged(tPrmSqn))
				cExecHintMin->item(tSqn) -= mPkWghts[thePos] *
						mPrOpfs[thePos](tPhRec->HintFlds[tPrmSqn].PrevBuff);
		}
	}
	CatchError
}
...
*/
template <typename pkt, typename pxt, typename fmt>
void WghtSumKXOFcMHnT<pkt,pxt,fmt>::execIncr()
{
	WatchError
	#if ExecUpwd
	...
	else
	{
		...
		for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
		{
			...
			if (this->mEvalUndonePrms->tagged(tPos))
			{
				...
				if (!this->mHintNotEvals[tPos])
				{
					EvalRec<pxt> const * tPeRec = mPoRecs[tPos].EvalRec;
					for(Sqn tPrmSqn = 0; tPrmSqn < this->PrmVars[tPos].itemCount(); ++tPrmSqn)
					{
						Sqn const tSqn = this->PrmVars[tPos][tPrmSqn];
						cExecHintMin->item(tSqn) += mPkWghts[tPos] *
			...
			if (this->mHintNotEvals[tPos])
			{
				HintRec<pxt> const * tPhRec = mPoRecs[tPos].HintRec;
				for (Idx sIdx = 0; sIdx < tPhRec->DiffSqns.itemCount(); ++sIdx)
				{
					Sqn const tPrmSqn = tPhRec->DiffSqns[sIdx];
					Sqn const tSqn = this->PrmVars[tPos][tPrmSqn];
...
*/
template <typename pkt, typename pxt, typename fmt>
void WghtSumKXOFcMHnT<pkt,pxt,fmt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pkt, typename pxt, typename fmt>
void WghtSumKXOFcMHnT<pkt,pxt,fmt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pkt, typename pxt, typename fmt>
void WghtSumKXOFcMHnT<pkt,pxt,fmt>::emulBklg()
{
	WatchError
