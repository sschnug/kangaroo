/*!
	@file sumxefcmhnt.tcc
	@brief The prototype file for SumXEFcMHnT template class.
	@details This is the prototype file for SumXEFcMHnT template class.
...
#ifndef SumXEFcMHnTTccIncluded
#define SumXEFcMHnTTccIncluded
...
class SumXEFcMHnT : public AggEFcMHnT<fmt>
{
	public:

		static Hdl def(Fss const theFss, Hdl const hdlSys, Prm * thePrms, Dim const PrmCount);	//!< Define a sum fcmhn.

		static SumXEFcMHnT<pxt,fmt> & refm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Reference.
		static SumXEFcMHnT<pxt,fmt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Pointer.
		static SumXEFcMHnT<pxt,fmt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static SumXEFcMHnT<pxt,fmt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static SumXEFcMHnT<pxt,fmt> const & cast(Term const & theTerm);	//!< Perform casting.
		static SumXEFcMHnT<pxt,fmt> const * cast(Term const * theTerm);	//!< Perform casting.
		static SumXEFcMHnT<pxt,fmt> & cast(Term & theTerm);				//!< Perform casting.
		static SumXEFcMHnT<pxt,fmt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		SumXEFcMHnT(Fss const theFss, Hdl const hdlSys,
									Prm * thePrms, Dim const PrmCount);		//!< Constructor.
		SumXEFcMHnT(SumXEFcMHnT<pxt,fmt> const & that);						//!< Duplicator.
		SumXEFcMHnT const & operator = (SumXEFcMHnT<pxt,fmt> const & that);	//!< Assigner.
		~SumXEFcMHnT();														//!< Destructor.
...
*/
template <typename pxt, typename fmt>
inline Bll SumXEFcMHnT<pxt,fmt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< SumXEFcMHnT<pxt,fmt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline Bll SumXEFcMHnT<pxt,fmt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< SumXEFcMHnT<pxt,fmt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fmt>
inline SumXEFcMHnT<pxt,fmt> const & SumXEFcMHnT<pxt,fmt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< SumXEFcMHnT<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<SumXEFcMHnT<pxt,fmt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline SumXEFcMHnT<pxt,fmt> & SumXEFcMHnT<pxt,fmt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< SumXEFcMHnT<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<SumXEFcMHnT<pxt,fmt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline SumXEFcMHnT<pxt,fmt> & SumXEFcMHnT<pxt,fmt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline SumXEFcMHnT<pxt,fmt> * SumXEFcMHnT<pxt,fmt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline SumXEFcMHnT<pxt,fmt> const & SumXEFcMHnT<pxt,fmt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline SumXEFcMHnT<pxt,fmt> const * SumXEFcMHnT<pxt,fmt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline SumXEFcMHnT<pxt,fmt>::~SumXEFcMHnT()
{
	WatchError
...
*/
template <typename pxt, typename fmt>
Hdl SumXEFcMHnT<pxt,fmt>::def(Fss const theFss, Hdl const hdlSys, Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 1 || PrmCount > MaxDim, eParamCount2(1, MaxDim));

	SumXEFcMHnT<pxt,fmt> * tFcMHn = new SumXEFcMHnT<pxt,fmt>(theFss, hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fmt>
SumXEFcMHnT<pxt,fmt>::SumXEFcMHnT(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount) :
		AggEFcMHnT<fmt>(hdlSys, PrmCount), mPxRecs(PrmCount),
		mPeRecs(PrmCount), mPrOpfs(PrmCount), cExecHintMin(Null),
		sChgdHintMin(Null)
{
	WatchError
	this->setTermCls(TermStub< SumXEFcMHnT<pxt,fmt> >::TermCls);
...
*/
template <typename pxt, typename fmt>
SumXEFcMHnT<pxt,fmt>::SumXEFcMHnT(SumXEFcMHnT const & that) : AggEFcMHnT<fmt>(that),
		mPxRecs(0), mPeRecs(0), mPrOpfs(0)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
SumXEFcMHnT<pxt,fmt> const & SumXEFcMHnT<pxt,fmt>::operator = (SumXEFcMHnT<pxt,fmt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void SumXEFcMHnT<pxt,fmt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fmt>
void SumXEFcMHnT<pxt,fmt>::execAnew()
{
	WatchError
	cExecMetric = 0;
	for(Sqn tSqn = 0; tSqn < this->FuncVars.itemCount(); ++tSqn)
...
*/
template <typename pxt, typename fmt>
void SumXEFcMHnT<pxt,fmt>::simulAnew()
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void SumXEFcMHnT<pxt,fmt>::undoEval(Pos const thePos)
{
	WatchError
	...
	for(Sqn tPrmSqn = 0; tPrmSqn < this->PrmVars[thePos].itemCount(); ++tPrmSqn)
	{
		...
		cExecHintMin->item(tSqn) -= mPrOpfs[thePos](
					mPeRecs[thePos]->PrevData(this->mTermSys.ExecClk()));
	}
	CatchError
}
#endif // CompLazy
...
*/
template <typename pxt, typename fmt>
void SumXEFcMHnT<pxt,fmt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fmt>
void SumXEFcMHnT<pxt,fmt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fmt>
void SumXEFcMHnT<pxt,fmt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fmt>
void SumXEFcMHnT<pxt,fmt>::emulBklg()
{
	WatchError
