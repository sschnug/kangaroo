/*!
	@file noruxfcmt.tcc
	@brief The prototype file for NoruXFcMt template class.
	@details This is the prototype file for NoruXFcMt template class.
...
#ifndef NoruXFcMtTccInlcluded
#define NoruXFcMtTccInlcluded
...
/*!
	@class NoruXFcMt
	@brief A class to represent nor (unsatisfaction) fcms.
	@details This class represents nor (unsatisfaction) fcms.
*/
template <typename pxt, typename fmt>
class NoruXFcMt : public AggFcMt<fmt>
{
	public:
		...
		static NoruXFcMt<pxt,fmt> & refm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Reference.
		static NoruXFcMt<pxt,fmt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Pointer.
		static NoruXFcMt<pxt,fmt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static NoruXFcMt<pxt,fmt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static NoruXFcMt<pxt,fmt> const & cast(Term const & theTerm);	//!< Perform casting.
		static NoruXFcMt<pxt,fmt> const * cast(Term const * theTerm);	//!< Perform casting.
		static NoruXFcMt<pxt,fmt> & cast(Term & theTerm);				//!< Perform casting.
		static NoruXFcMt<pxt,fmt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~NoruXFcMt();														//!< Destructor.
		NoruXFcMt(NoruXFcMt<pxt,fmt> const & that);							//!< Duplicator.
		NoruXFcMt(Fss const theFss, Hdl const hdlSys,
										Prm * thePrms, Dim const PrmCount);	//!< Constructor.
		NoruXFcMt const & operator = (NoruXFcMt<pxt,fmt> const & that);		//!< Assigner.
...
*/
template <typename pxt, typename fmt>
inline Bll NoruXFcMt<pxt,fmt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< NoruXFcMt<pxt,fmt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline Bll NoruXFcMt<pxt,fmt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< NoruXFcMt<pxt,fmt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fmt>
inline NoruXFcMt<pxt,fmt> const & NoruXFcMt<pxt,fmt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< NoruXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<NoruXFcMt<pxt,fmt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline NoruXFcMt<pxt,fmt> & NoruXFcMt<pxt,fmt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< NoruXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<NoruXFcMt<pxt,fmt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline NoruXFcMt<pxt,fmt> & NoruXFcMt<pxt,fmt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline NoruXFcMt<pxt,fmt> * NoruXFcMt<pxt,fmt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline NoruXFcMt<pxt,fmt> const & NoruXFcMt<pxt,fmt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline NoruXFcMt<pxt,fmt> const * NoruXFcMt<pxt,fmt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline NoruXFcMt<pxt,fmt>::~NoruXFcMt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fmt>
Hdl NoruXFcMt<pxt,fmt>::def(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	NoruXFcMt<pxt,fmt> * tFcM = new NoruXFcMt<pxt,fmt>(theFss,hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fmt>
NoruXFcMt<pxt,fmt>::NoruXFcMt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount) :
		AggFcMt<fmt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< NoruXFcMt<pxt,fmt> >::TermCls);
...
*/
template <typename pxt, typename fmt>
NoruXFcMt<pxt,fmt>::NoruXFcMt(NoruXFcMt const & that) : AggFcMt<fmt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
NoruXFcMt<pxt,fmt> const & NoruXFcMt<pxt,fmt>::operator = (NoruXFcMt<pxt,fmt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void NoruXFcMt<pxt,fmt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fmt>
void NoruXFcMt<pxt,fmt>::execAnew()
{
	WatchError
	cExecTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fmt>
void NoruXFcMt<pxt,fmt>::simulAnew()
{
	WatchError
	Dim tSimulTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fmt>
void NoruXFcMt<pxt,fmt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void NoruXFcMt<pxt,fmt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fmt>
void NoruXFcMt<pxt,fmt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fmt>
void NoruXFcMt<pxt,fmt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fmt>
void NoruXFcMt<pxt,fmt>::emulBklg()
{
	WatchError
	 cEmulTrue = cExecTrue;
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
