/*!
	@file tcountxfcmt.tcc
	@brief The prototype file for TcountXFcMt template class.
	@details This is the prototype file for TcountXFcMt template class.
...
#ifndef TcountXFcMtTccInlcluded
#define TcountXFcMtTccInlcluded
...
/*!
	@class TcountXFcMt
	@brief A class to represent true count fcms.
	@details This class represents true count fcms.
*/
template <typename pxt, typename fmt>
class TcountXFcMt : public AggFcMt<fmt>
{
	public:

		static Hdl def(Fss const theFss, Hdl const hdlSys,
												Prm * thePrms, Dim const PrmCount);		//!< Definition.
		static TcountXFcMt<pxt,fmt> & refm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Reference.
		static TcountXFcMt<pxt,fmt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Pointer.
		static TcountXFcMt<pxt,fmt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static TcountXFcMt<pxt,fmt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static TcountXFcMt<pxt,fmt> const & cast(Term const & theTerm);	//!< Perform casting.
		static TcountXFcMt<pxt,fmt> const * cast(Term const * theTerm);	//!< Perform casting.
		static TcountXFcMt<pxt,fmt> & cast(Term & theTerm);				//!< Perform casting.
		static TcountXFcMt<pxt,fmt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~TcountXFcMt();														//!< Destructor.
		TcountXFcMt(TcountXFcMt<pxt,fmt> const & that);						//!< Duplicator.
		TcountXFcMt(Fss const theFss, Hdl const hdlSys,
										Prm * thePrms, Dim const PrmCount);	//!< Constructor.
		TcountXFcMt const & operator = (TcountXFcMt<pxt,fmt> const & that);	//!< Assigner.
...
*/
template <typename pxt, typename fmt>
inline Bll TcountXFcMt<pxt,fmt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< TcountXFcMt<pxt,fmt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline Bll TcountXFcMt<pxt,fmt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< TcountXFcMt<pxt,fmt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fmt>
inline TcountXFcMt<pxt,fmt> const & TcountXFcMt<pxt,fmt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< TcountXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<TcountXFcMt<pxt,fmt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline TcountXFcMt<pxt,fmt> & TcountXFcMt<pxt,fmt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< TcountXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<TcountXFcMt<pxt,fmt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline TcountXFcMt<pxt,fmt> & TcountXFcMt<pxt,fmt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline TcountXFcMt<pxt,fmt> * TcountXFcMt<pxt,fmt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline TcountXFcMt<pxt,fmt> const & TcountXFcMt<pxt,fmt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline TcountXFcMt<pxt,fmt> const * TcountXFcMt<pxt,fmt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline TcountXFcMt<pxt,fmt>::~TcountXFcMt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fmt>
Hdl TcountXFcMt<pxt,fmt>::def(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	TcountXFcMt<pxt,fmt> * tFcM = new TcountXFcMt<pxt,fmt>(theFss,hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fmt>
TcountXFcMt<pxt,fmt>::TcountXFcMt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount) :
		AggFcMt<fmt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< TcountXFcMt<pxt,fmt> >::TermCls);
...
*/
template <typename pxt, typename fmt>
TcountXFcMt<pxt,fmt>::TcountXFcMt(TcountXFcMt const & that) : AggFcMt<fmt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
TcountXFcMt<pxt,fmt> const & TcountXFcMt<pxt,fmt>::operator = (TcountXFcMt<pxt,fmt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void TcountXFcMt<pxt,fmt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fmt>
void TcountXFcMt<pxt,fmt>::execAnew()
{
	WatchError
	cExecTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fmt>
void TcountXFcMt<pxt,fmt>::simulAnew()
{
	WatchError
	Dim tSimulTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fmt>
void TcountXFcMt<pxt,fmt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void TcountXFcMt<pxt,fmt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fmt>
void TcountXFcMt<pxt,fmt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fmt>
void TcountXFcMt<pxt,fmt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fmt>
void TcountXFcMt<pxt,fmt>::emulBklg()
{
	WatchError
	 cEmulTrue = cExecTrue;
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
