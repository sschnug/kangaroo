/*!
	@file anduxfcmt.tcc
	@brief The prototype file for AnduXFcMt template class.
	@details This is the prototype file for AnduXFcMt template class.
...
#ifndef AnduXFcMtTccIncluded
#define AnduXFcMtTccIncluded
...
/*!
	@class AnduXFcMt
	@brief A class to represent and (unsatisfaction) fcms.
	@details This class represents and (unsatisfaction) fcms.
*/
template <typename pxt, typename fmt>
class AnduXFcMt : public AggFcMt<fmt>
{
	public:
		static Hdl def(Fss const theFss, Hdl const hdlSys,
												Prm * thePrms, Dim const PrmCount);		//!< Definition.

		static AnduXFcMt<pxt,fmt> & refm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Reference.
		static AnduXFcMt<pxt,fmt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Pointer.
		static AnduXFcMt<pxt,fmt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static AnduXFcMt<pxt,fmt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static AnduXFcMt<pxt,fmt> const & cast(Term const & theTerm);	//!< Perform casting.
		static AnduXFcMt<pxt,fmt> const * cast(Term const * theTerm);	//!< Perform casting.
		static AnduXFcMt<pxt,fmt> & cast(Term & theTerm);				//!< Perform casting.
		static AnduXFcMt<pxt,fmt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~AnduXFcMt();													//!< Destructor.
		AnduXFcMt(AnduXFcMt<pxt,fmt> const & that);						//!< Duplicator.
		AnduXFcMt(Fss const theFss, Hdl const hdlSys,
									Prm * thePrms, Dim const PrmCount);	//!< Constructor.
		AnduXFcMt const & operator = (AnduXFcMt<pxt,fmt> const & that);	//!< Assigner.
...
*/
template <typename pxt, typename fmt>
inline Bll AnduXFcMt<pxt,fmt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< AnduXFcMt<pxt,fmt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline Bll AnduXFcMt<pxt,fmt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< AnduXFcMt<pxt,fmt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fmt>
inline AnduXFcMt<pxt,fmt> const & AnduXFcMt<pxt,fmt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< AnduXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<AnduXFcMt<pxt,fmt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline AnduXFcMt<pxt,fmt> & AnduXFcMt<pxt,fmt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< AnduXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<AnduXFcMt<pxt,fmt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline AnduXFcMt<pxt,fmt> & AnduXFcMt<pxt,fmt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline AnduXFcMt<pxt,fmt> * AnduXFcMt<pxt,fmt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline AnduXFcMt<pxt,fmt> const & AnduXFcMt<pxt,fmt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline AnduXFcMt<pxt,fmt> const * AnduXFcMt<pxt,fmt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline AnduXFcMt<pxt,fmt>::~AnduXFcMt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fmt>
Hdl AnduXFcMt<pxt,fmt>::def(Fss const theFss, Hdl const hdlSys,
			Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	AnduXFcMt<pxt,fmt> * tFcM = new AnduXFcMt<pxt,fmt>(theFss, hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fmt>
AnduXFcMt<pxt,fmt>::AnduXFcMt(Fss const theFss, Hdl const hdlSys,
			Prm * thePrms, Dim const PrmCount) :
		AggFcMt<fmt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< AnduXFcMt<pxt,fmt> >::TermCls);
...
*/
template <typename pxt, typename fmt>
AnduXFcMt<pxt,fmt>::AnduXFcMt(AnduXFcMt const & that) : AggFcMt<fmt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
AnduXFcMt<pxt,fmt> const & AnduXFcMt<pxt,fmt>::operator = (AnduXFcMt<pxt,fmt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void AnduXFcMt<pxt,fmt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fmt>
void AnduXFcMt<pxt,fmt>::execAnew()
{
	WatchError
	cExecFalse = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fmt>
void AnduXFcMt<pxt,fmt>::simulAnew()
{
	WatchError
	Dim tSimulFalse = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fmt>
void AnduXFcMt<pxt,fmt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void AnduXFcMt<pxt,fmt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fmt>
void AnduXFcMt<pxt,fmt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fmt>
void AnduXFcMt<pxt,fmt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fmt>
void AnduXFcMt<pxt,fmt>::emulBklg()
{
	WatchError
