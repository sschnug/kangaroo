/*!
	@file orsxfcmt.tcc
	@brief The prototype file for OrsXFcMt template class.
	@details This is the prototype file for OrsXFcMt template class.
...
#ifndef OrsXFcMtTccInlcluded
#define OrsXFcMtTccInlcluded
...
/*!
	@class OrsXFcMt
	@brief A class to represent or (satisfaction) fcms.
	@details This class represents or (satisfaction) fcms.
*/
template <typename pxt, typename fmt>
class OrsXFcMt : public AggFcMt<fmt>
{
	public:
		...
		static OrsXFcMt<pxt,fmt> & refm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Reference.
		static OrsXFcMt<pxt,fmt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);		//!< Pointer.
		static OrsXFcMt<pxt,fmt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static OrsXFcMt<pxt,fmt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static OrsXFcMt<pxt,fmt> const & cast(Term const & theTerm);	//!< Perform casting.
		static OrsXFcMt<pxt,fmt> const * cast(Term const * theTerm);	//!< Perform casting.
		static OrsXFcMt<pxt,fmt> & cast(Term & theTerm);				//!< Perform casting.
		static OrsXFcMt<pxt,fmt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~OrsXFcMt();													//!< Destructor.
		OrsXFcMt(OrsXFcMt<pxt,fmt> const & that);						//!< Duplicator.
		OrsXFcMt(Fss const theFss, Hdl const hdlSys,
								Prm * thePrms, Dim const PrmCount);		//!< Constructor.
		OrsXFcMt const & operator = (OrsXFcMt<pxt,fmt> const & that);	//!< Assigner.
...
*/
template <typename pxt, typename fmt>
inline Bll OrsXFcMt<pxt,fmt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< OrsXFcMt<pxt,fmt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline Bll OrsXFcMt<pxt,fmt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< OrsXFcMt<pxt,fmt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fmt>
inline OrsXFcMt<pxt,fmt> const & OrsXFcMt<pxt,fmt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< OrsXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<OrsXFcMt<pxt,fmt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline OrsXFcMt<pxt,fmt> & OrsXFcMt<pxt,fmt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< OrsXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<OrsXFcMt<pxt,fmt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline OrsXFcMt<pxt,fmt> & OrsXFcMt<pxt,fmt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline OrsXFcMt<pxt,fmt> * OrsXFcMt<pxt,fmt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline OrsXFcMt<pxt,fmt> const & OrsXFcMt<pxt,fmt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline OrsXFcMt<pxt,fmt> const * OrsXFcMt<pxt,fmt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline OrsXFcMt<pxt,fmt>::~OrsXFcMt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fmt>
Hdl OrsXFcMt<pxt,fmt>::def(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	OrsXFcMt<pxt,fmt> * tFcM = new OrsXFcMt<pxt,fmt>(theFss,hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fmt>
OrsXFcMt<pxt,fmt>::OrsXFcMt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount) :
		AggFcMt<fmt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< OrsXFcMt<pxt,fmt> >::TermCls);
...
*/
template <typename pxt, typename fmt>
OrsXFcMt<pxt,fmt>::OrsXFcMt(OrsXFcMt const & that) : AggFcMt<fmt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
OrsXFcMt<pxt,fmt> const & OrsXFcMt<pxt,fmt>::operator = (OrsXFcMt<pxt,fmt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void OrsXFcMt<pxt,fmt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
		...
		this->mMetricRec->finalise(castB(cExecTrue));
	}
	CatchError
}
...
*/
template <typename pxt, typename fmt>
void OrsXFcMt<pxt,fmt>::execAnew()
{
	WatchError
	cExecTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
	...
	this->mMetricRec->initCurr(castB(cExecTrue));
	#if ExecUpwdLazy
...
*/
template <typename pxt, typename fmt>
void OrsXFcMt<pxt,fmt>::simulAnew()
{
	WatchError
	Dim tSimulTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
	...
	this->mMetricRec->updtNext(this->mTermSys.SimulClk(), castB(tSimulTrue));
	#if SimulUpwd
...
*/
template <typename pxt, typename fmt>
void OrsXFcMt<pxt,fmt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void OrsXFcMt<pxt,fmt>::execIncr()
{
	WatchError
	#if ExecUpwd
	if (this->mExecChgdPrms.tagCount())
	{
		...
		this->mMetricRec->updtCurr(this->mTermSys.ExecClk(), castB(cExecTrue));
...
*/
template <typename pxt, typename fmt>
void OrsXFcMt<pxt,fmt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fmt>
void OrsXFcMt<pxt,fmt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fmt>
void OrsXFcMt<pxt,fmt>::emulBklg()
{
	WatchError
	 cEmulTrue = cExecTrue;
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	...
	this->mMetricRec->updtLate(this->mTermSys.EmulClk(), castB(cEmulTrue));
	CatchError
}
#endif //CompLazy
