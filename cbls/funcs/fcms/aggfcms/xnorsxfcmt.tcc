/*!
	@file xnorsxfcmt.tcc
	@brief The prototype file for XnorsXFcMt template class.
	@details This is the prototype file for XnorsXFcMt template class.
...
#ifndef XnorsXFcMtTccInlcluded
#define XnorsXFcMtTccInlcluded
...
class XnorsXFcMt : public AggFcMt<fmt>
{
	public:

		static Hdl def(Fss const theFss, Hdl const hdlSys,
												Prm * thePrms, Dim const PrmCount);		//!< Definition.
		static XnorsXFcMt<pxt,fmt> & refm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Reference.
		static XnorsXFcMt<pxt,fmt> * ptrm(Hdl const hdlSys, Hdl const hdlFunc);			//!< Pointer.
		static XnorsXFcMt<pxt,fmt> const & refc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Reference.
		static XnorsXFcMt<pxt,fmt> const * ptrc(Hdl const hdlSys, Hdl const hdlFunc);	//!< Pointer.

		//! @name Casting
		//! @{

		static XnorsXFcMt<pxt,fmt> const & cast(Term const & theTerm);	//!< Perform casting.
		static XnorsXFcMt<pxt,fmt> const * cast(Term const * theTerm);	//!< Perform casting.
		static XnorsXFcMt<pxt,fmt> & cast(Term & theTerm);				//!< Perform casting.
		static XnorsXFcMt<pxt,fmt> * cast(Term * theTerm);				//!< Perform casting.
	...
	private:
		...
		~XnorsXFcMt();														//!< Destructor.
		XnorsXFcMt(XnorsXFcMt<pxt,fmt> const & that);						//!< Duplicator.
		XnorsXFcMt(Fss const theFss, Hdl const hdlSys,
										Prm * thePrms, Dim const PrmCount);	//!< Constructor.
		XnorsXFcMt const & operator = (XnorsXFcMt<pxt,fmt> const & that);	//!< Assigner.
...
*/
template <typename pxt, typename fmt>
inline Bll XnorsXFcMt<pxt,fmt>::suit(Term const & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< XnorsXFcMt<pxt,fmt> >::TermCls;
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline Bll XnorsXFcMt<pxt,fmt>::suit(Term & theTerm)
{
	WatchError
	return theTerm.TermCls == TermStub< XnorsXFcMt<pxt,fmt> >::TermCls;
	CatchError
...
*/
template <typename pxt, typename fmt>
inline XnorsXFcMt<pxt,fmt> const & XnorsXFcMt<pxt,fmt>::cast(Term const & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< XnorsXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<XnorsXFcMt<pxt,fmt> const &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline XnorsXFcMt<pxt,fmt> & XnorsXFcMt<pxt,fmt>::cast(Term & theTerm)
{
	WatchError
	Wchk(theTerm.TermCls != TermStub< XnorsXFcMt<pxt,fmt> >::TermCls) Wtxt(eWrongCls);

	return scast<XnorsXFcMt<pxt,fmt> &>(theTerm);
	CatchError
}
...
*/
template <typename pxt, typename fmt>
inline XnorsXFcMt<pxt,fmt> & XnorsXFcMt<pxt,fmt>::refm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline XnorsXFcMt<pxt,fmt> * XnorsXFcMt<pxt,fmt>::ptrm(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline XnorsXFcMt<pxt,fmt> const & XnorsXFcMt<pxt,fmt>::refc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline XnorsXFcMt<pxt,fmt> const * XnorsXFcMt<pxt,fmt>::ptrc(Hdl const hdlSys, Hdl const hdlFunc)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
inline XnorsXFcMt<pxt,fmt>::~XnorsXFcMt()
{
	WatchError
	//	nothing to be done.
	CatchError
...
*/
template <typename pxt, typename fmt>
Hdl XnorsXFcMt<pxt,fmt>::def(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount)
{
	WatchError
	Warn(PrmCount < 2 || PrmCount > MaxDim, eParamCount2(2, MaxDim));

	XnorsXFcMt<pxt,fmt> * tFcM = new XnorsXFcMt<pxt,fmt>(theFss,hdlSys, thePrms, PrmCount);
...
*/
template <typename pxt, typename fmt>
XnorsXFcMt<pxt,fmt>::XnorsXFcMt(Fss const theFss, Hdl const hdlSys,
		Prm * thePrms, Dim const PrmCount) :
		AggFcMt<fmt>(hdlSys, PrmCount), mPxRecs(PrmCount)
{
	WatchError
	this->setTermCls(TermStub< XnorsXFcMt<pxt,fmt> >::TermCls);
...
*/
template <typename pxt, typename fmt>
XnorsXFcMt<pxt,fmt>::XnorsXFcMt(XnorsXFcMt const & that) : AggFcMt<fmt>(that), mPxRecs(0)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
XnorsXFcMt<pxt,fmt> const & XnorsXFcMt<pxt,fmt>::operator = (XnorsXFcMt<pxt,fmt> const & that)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void XnorsXFcMt<pxt,fmt>::compConst()
{
	WatchError
	if (!this->updatable())
	{
...
*/
template <typename pxt, typename fmt>
void XnorsXFcMt<pxt,fmt>::execAnew()
{
	WatchError
	cExecTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fmt>
void XnorsXFcMt<pxt,fmt>::simulAnew()
{
	WatchError
	Dim tSimulTrue = 0;
	for(Pos tPos = 0; tPos < mPxRecs.itemCount(); ++tPos)
...
*/
template <typename pxt, typename fmt>
void XnorsXFcMt<pxt,fmt>::undoEval(Pos const thePos)
{
	WatchError
...
*/
template <typename pxt, typename fmt>
void XnorsXFcMt<pxt,fmt>::execIncr()
{
	WatchError
	#if ExecUpwd
...
*/
template <typename pxt, typename fmt>
void XnorsXFcMt<pxt,fmt>::execBklg()
{
	WatchError
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
	{
...
*/
template <typename pxt, typename fmt>
void XnorsXFcMt<pxt,fmt>::simulIncr()
{
	WatchError
	#if SimulUpwd
...
*/
template <typename pxt, typename fmt>
void XnorsXFcMt<pxt,fmt>::emulBklg()
{
	WatchError
	 cEmulTrue = cExecTrue;
	for(Idx tIdx = 0; tIdx < this->mBacklogPrms.tagCount(); ++tIdx)
